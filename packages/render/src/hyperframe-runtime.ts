...
import { dimsFor } from "@showtell/compose";
import {
...
function runtimeImportSource(source: string, sdkFile: string): string {
  return source.replaceAll(/from\s+["']@showtell\/hyperframes["']/g, `from "./${sdkFile}"`);
}
...
async function loadHyperframeModule(
  ...
  const runtimeDir = join(bundleDir, ".showtell", "hyperframe-runtime");
