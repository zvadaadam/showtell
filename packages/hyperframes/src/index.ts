...
export type ThemePreset = "ink" | "aurora" | "ember" | "orchid" | "graphite" | "agent-dark" | "paper" | "neutral";
...
export interface HyperframeThemeColors {
  ...
  subtle: string;
  accent: string;
  /** Secondary hue for background glows; renderers fall back to accent. */
  accent2?: string;
  success: string;
  warning: string;
  surface: string;
...
export interface BigStatProps {
  /** The headline figure, e.g. "183", "4.9×", "12ms". */
  value: string;
  label?: string;
  /** Short delta text, e.g. "+31%" or "-120ms". Leading "-"/"−" points the arrow down. */
  delta?: string;
  deltaTone?: "success" | "warning" | "info";
  align?: "left" | "center";
}
...
export interface TravelPathPoint {
  /** Normalized 0..1 position inside the component box. */
  x: number;
  y: number;
  label?: string;
}
...
export interface FunctionPlotProps {
  /** Curve samples in ascending x (compute pure math inline; it is deterministic). */
  points: FunctionPlotPoint[];
  /** Waypoints a marker ball travels by arc length (e.g. gradient-descent steps). */
  path?: FunctionPlotPoint[];
  /** 0..1 along `path`; map to a range for narration-synced travel. */
  progress?: number;
  /** Draw a tangent segment at the ball (slope from the sampled curve). */
  tangent?: boolean;
  /** Dots on waypoints already reached (default true). */
  showSteps?: boolean;
  xLabel?: string;
  yLabel?: string;
}
...
export interface FormulaProps {
  /** Equation text; the pinned mono face covers θ α ∇ ← ≈ ∂. */
  text: string;
  /** Substrings rendered in the accent color (drive from ctx.scene.lineIndex). */
  highlight?: string[];
  caption?: string;
}

export interface TravelPathProps {
  from: TravelPathPoint;
  to: TravelPathPoint;
  /**
   * 0..1 travel progress. Map it to a range (`ctx.range("flight").progress`)
   * for narration-synced travel; omit for an automatic ~2s flight on scene entry.
   */
  progress?: number;
  /** Arc bend, -1..1. Positive arcs over a left-to-right route. Default 0.35. */
  curve?: number;
  marker?: "plane" | "dot";
}
...
export const BigStat = component<BigStatProps>("BigStat");
export const Checklist = component<ChecklistProps>("Checklist");
export const Quote = component<QuoteProps>("Quote");
export const TravelPath = component<TravelPathProps>("TravelPath");
export const FunctionPlot = component<FunctionPlotProps>("FunctionPlot");
export const Formula = component<FormulaProps>("Formula");
...
export const SignalWall: HyperframeComponent<SignalWallProps> = (props) => {
  ...
  return h(
    Grid,
    { columns: props.columns ?? 2, gap: "sm", grow: true },
...
export const LaneStack: HyperframeComponent<LaneStackProps> = (props) => {
  ...
  return h(
    Stack,
    { direction: "vertical", gap: "sm", grow: true },
    lanes.map((lane, index) =>
      h(
        Panel,
        { tone: index <= active ? "accent" : "default", padding: "sm", grow: true },
...
export const DecisionGrid: HyperframeComponent<DecisionGridProps> = (props) => {
  ...
  return h(
    Grid,
    { columns: props.columns ?? Math.min(3, options.length), gap: "md", grow: true },
...
export const ProofLadder: HyperframeComponent<ProofLadderProps> = (props) => {
  ...
  return h(
    ...
    items.map((item, index) =>
      h(
        Stack,
        { direction: "vertical", gap: "xs", grow: true },
        h(Meter, {
          label: item.label,
...
export const StatRow: HyperframeComponent<StatRowProps> = (props) => {
  const stats = props.stats.length ? props.stats : [{ value: "0", label: "stat" }];
  // When any card carries a delta, reserve (blank) delta space on the others so
  // the big values sit on one shared line across the row.
  const anyDelta = stats.some((stat) => typeof stat.delta === "string" && stat.delta.trim() !== "");
  return h(
    Grid,
    { columns: props.columns ?? Math.min(4, stats.length), gap: "sm", grow: true },
    stats.map((stat) =>
      h(
        Panel,
        { padding: "sm" },
        h(BigStat, { align: "center", ...stat, delta: stat.delta ?? (anyDelta ? " " : undefined) }),
      ),
    ),
  );
};
...
export interface CompareSplitProps {
  beforeTitle?: string;
  before: (string | ChecklistItemInput)[];
  afterTitle?: string;
  after: (string | ChecklistItemInput)[];
  activeSide?: "before" | "after";
}

function checklistItems(items: (string | ChecklistItemInput)[], state: ChecklistState): ChecklistItemInput[] {
  return items.map((item) => (typeof item === "string" ? { label: item, state } : { state, ...item }));
}

export const CompareSplit: HyperframeComponent<CompareSplitProps> = (props) => {
  const active = props.activeSide ?? "after";
  return h(
    Stack,
    { direction: "horizontal", gap: "md", grow: true },
    h(
      Panel,
      { title: props.beforeTitle ?? "Before", padding: "md", tone: active === "before" ? "warning" : "default" },
      h(Checklist, { items: checklistItems(props.before, "todo") }),
    ),
    h(
      Panel,
      { title: props.afterTitle ?? "After", padding: "md", tone: active === "after" ? "success" : "default" },
      h(Checklist, { items: checklistItems(props.after, "done") }),
    ),
  );
};
...
export const hyperframeComponents: HyperframeComponentManifest[] = [
  ...
  },
  {
    ...
    importName: "BigStat",
    layer: "host",
    description: "One huge headline figure with an optional label and delta arrow.",
    bestFor: ["metric moments", "before/after numbers", "test counts", "latency wins"],
    requiredProps: ["value"],
    commonProps: ["label", "delta", "deltaTone", "align"],
    example: '<BigStat value="183" label="tests passing" delta="+12" deltaTone="success" />',
  },
  {
    id: "checklist",
    title: "Checklist",
    ...
    layer: "host",
    description: "Drawn check circles with done/active/todo states and optional detail lines.",
    bestFor: ["shipped lists", "release notes", "todo vs done", "step completion"],
    requiredProps: ["items"],
    commonProps: ["columns"],
    example: '<Checklist items={[{ label: "Layout engine", state: "done" }]} />',
  },
  {
    id: "quote",
    title: "Quote",
    importName: "Quote",
    layer: "host",
    description: "A pull-quote with a decorative mark and optional attribution.",
    bestFor: ["user feedback", "reviews", "design principles", "docs excerpts"],
    requiredProps: ["text"],
    commonProps: ["attribution"],
    example: '<Quote text="Frames that design themselves." attribution="release notes" />',
  },
  {
    id: "function-plot",
    title: "Function Plot",
    importName: "FunctionPlot",
    layer: "host",
    description:
      "Numeric x/y plot with gridlines, tick labels, an area-faded curve, and an optional waypoint path a marker ball travels (with tangent). The math-education workhorse.",
    bestFor: ["gradient descent", "loss curves", "any y = f(x) explanation", "convergence stories"],
    requiredProps: ["points"],
    commonProps: ["path", "progress", "tangent", "xLabel", "yLabel"],
    example:
      '<FunctionPlot points={samples} path={steps} progress={ctx.range("descent").progress} tangent xLabel="θ" yLabel="J(θ)" />',
  },
  {
    id: "formula",
    title: "Formula",
    ...
    layer: "host",
    description:
      "Display equation in the pinned mono face (θ α ∇ ← ≈ ∂ covered) with accent-highlighted terms and an optional caption.",
    bestFor: ["update rules", "definitions", "highlighting the term being narrated"],
    requiredProps: ["text"],
    commonProps: ["highlight", "caption"],
    example:
      '<Formula text="θ ← θ − α · ∇J(θ)" highlight={ctx.scene.lineIndex > 0 ? ["α"] : []} caption="the learning rate scales every step" />',
  },
  {
    id: "travel-path",
    title: "Travel Path",
    importName: "TravelPath",
    layer: "host",
    description:
      "Animated route between two normalized points: dashed guide, traveled accent line, labeled endpoints, and a plane/dot marker along the arc. Layer it over an ImageAsset map for journeys.",
    bestFor: ["journeys (PRG → SF)", "data flowing between systems", "request paths", "migration arcs"],
    requiredProps: ["from", "to"],
    commonProps: ["progress", "curve", "marker"],
    example:
      '<TravelPath from={{ x: 0.58, y: 0.32, label: "Prague" }} to={{ x: 0.1, y: 0.42, label: "San Francisco" }} progress={ctx.range("flight").progress} />',
  },
  {
  ...
  },
  {
    ...
    layer: "host",
    description:
      "Visual narration caption layer inside a hyperframe. word-pop reveals words with a scale-pop synced to the cue; karaoke highlights the active word; minimal/stacked render a static pill that rises in per cue. Emphasized words render in the theme accent color.",
  ...
  },
  {
    ...
    importName: "StatRow",
    layer: "story",
    description: "A row of big-stat cards for the numbers that carry the story.",
    bestFor: ["metric summaries", "release results", "benchmark rows"],
    requiredProps: ["stats"],
    commonProps: ["columns"],
    example: '<StatRow stats={[{ value: "183", label: "tests" }, { value: "0", label: "failures" }]} />',
  },
  {
    id: "compare-split",
    title: "Compare Split",
    importName: "CompareSplit",
    layer: "story",
    description: "Before/after panels with todo vs done checklists and one highlighted side.",
    bestFor: ["refactors", "migrations", "bug fixes", "design overhauls"],
    requiredProps: ["before", "after"],
    commonProps: ["beforeTitle", "afterTitle", "activeSide"],
    example: '<CompareSplit before={["Overlapping text"]} after={["Measured layout"]} />',
  },
  {
    id: "code-ref",
    title: "CodeRef",
