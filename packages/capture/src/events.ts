...
export interface BrowserAction {
  ...
  t: number;
  /** Optional uncertainty window for command wrappers. */
  startT?: number;
  endT?: number;
}
...
export function toCaptureEvent(a: BrowserAction, source: { width: number; height: number }): CaptureEvent {
  ...
  if (a.startT !== undefined) event.startT = Math.max(0, a.startT);
  if (a.endT !== undefined) event.endT = Math.max(0, a.endT);
  return event;
}
