...
function stageRootPackage(): string {
  ...
  writeManifest(stageDir, {
    ...
    files: ["bin/showtell.cjs", "README.md", "LICENSE"],
...
function stagePlatformPackage(target: ReleaseTarget, binarySource: string): string {
...
function compile(target: ReleaseTarget): string {
  ...
  run(["bun", "build", "packages/cli/src/index.ts", "--compile", `--target=${target.bunTarget}`, "--outfile", binary]);
  return binary;
}
...
Showtell is an open-source motion engine for videos made by agents, for humans. Agents author structured intent and deterministic hyperframes; Showtell renders narrated MP4s from code, diffs, data, maps, screenshots, images, and other declared inputs.
...
Showtell supports Apple Silicon macOS and glibc-based Linux on x64 or ARM64. It installs one self-contained native binary for your platform; Bun is not required. Rendering requires [ffmpeg](https://ffmpeg.org/).
...
if (binaryDir) {
  for (const target of RELEASE_TARGETS) {
    staged.push(stagePlatformPackage(target, join(binaryDir, `showtell-${target.id}`)));
  }
} else {
